...
const makeserialwise = () => {
    let i = 1;
    document.querySelectorAll(".list").forEach((e) => {
        e.querySelector(".serial").innerHTML = i;
        // e.classList[1] = `l${i}`;
        // // e.querySelector(".inp").classList[1] = `i${i}`;
        // e.querySelector(".edit").classList[1] = `e${i}`;
        // e.querySelector(".del").classList[1] = `d${i}`;
        i++;
    })
}
...
a.addEventListener("click", () => {
    count++;
    let x =
        `<div class="list l${count}">
        <div class="serial">${count}</div>

        <div class="name">
            <input type="text" class="inp i${count}" placeholder="Enter Task to do">       
        </div>

        <div class="check c${count}"><input type="checkbox"></div>
        
        <div class="edit e${count}">
    ...
    // for delete   
    //delete event  must be added in above event to add event on every list item

    const dlt = document.querySelectorAll(".del");
    dlt.forEach((e) => {    // foreach is used to add event listenner to all elements

        let cl = e.classList[1];
        // console.log(cl);

        e.addEventListener("click", () => {
            alert("Do you want to Delete the list ");
            document.body.querySelector(`.l${cl.slice(1)}`).remove()
    ...
    document.querySelectorAll(".edit").forEach((e) => {
        let cls = e.classList[1];
        e.addEventListener("click", () => {
            let text = document.body.querySelector(`.i${cls.slice(1)}`).value;
            document.body.querySelector(`.l${cls.slice(1)}`).querySelector(".name").replaceChildren(text[0].toLocaleUpperCase() + text.slice(1))
            e.outerHTML="";
    ...
    document.querySelectorAll(".check").forEach((e) => {
        ...
        e.addEventListener("click", () => {
            // let text = document.body.querySelector(`.i${cls.slice(1)}`).value;
            document.body.querySelector(`.l${cls.slice(1)}`).style.backgroundColor="#70f96182";
            e.innerHTML="";
        })
